...
def test_post_load_packs_final_source_and_preserves_address():
    events: list[str] = []
    ...
    first = base_conv.ascend_conv1d_weight
    first_ptr = first.data_ptr()
    expected = base_conv.weight.squeeze(1).transpose(0, 1).to(first.dtype)
    torch.testing.assert_close(first, expected)
    assert events == ["process"]

    with torch.no_grad():
        base_conv.weight.data.copy_(torch.full_like(base_conv.weight, 7.0))
    base_conv.quant_method.process_weights_after_loading(base_conv)

    assert base_conv.ascend_conv1d_weight.data_ptr() == first_ptr
    torch.testing.assert_close(
        base_conv.ascend_conv1d_weight,
        base_conv.weight.squeeze(1).transpose(0, 1).to(first.dtype),
    )


def test_packed_getter_reads_registered_parameter_and_resolves_lora_base():
    layer = _make_layer(_source_weight(), lora=True)
    initialize_packed_conv_weight(layer)

    assert _get_packed_conv_weights(layer) is layer.conv1d.base_layer.ascend_conv1d_weight
    assert _PACKED_CONV_WEIGHT_NAME not in layer.conv1d._parameters


def test_meta_weight_keeps_only_meta_placeholder():
...
def test_layerwise_reload_updates_source_and_packed_in_place():
    layer = _make_layer(_source_weight())
    ...
    packed_ptr = base_conv.ascend_conv1d_weight.data_ptr()
    ...
    assert base_conv.ascend_conv1d_weight.data_ptr() == packed_ptr
    torch.testing.assert_close(base_conv.weight, loaded)
    torch.testing.assert_close(
        base_conv.ascend_conv1d_weight,
        loaded.squeeze(1).transpose(0, 1).to(base_conv.ascend_conv1d_weight.dtype),
    )
